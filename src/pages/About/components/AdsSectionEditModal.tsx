...
export const AdsSectionEditModal = (props: adsSection) => {
  ...
  const handleUpdate = async () => {
    ...
    await updateFirebaseDoc(
      `/Empresa/ZktZQqsBnqVVoL4dfRHv/secciones/${forms.id}`,
      data
    );
    data = {
      ...data,
  ...
  return (
    <>
      <div className="justify-content-center">
        <button
          type="button"
          className="btn btn-outline-warning mb-1 btn-sm"
            ...
            <div className="modal-body">
              <div className="form-row text-black">
                <label
                  ...
                  htmlFor={`title-about-adsSection`}
                >
                  Título
                </label>
                <textarea
                  className="form-control rounded-0 h-10"
                  id={`title-about-${props.id}`}
                  name="titulo"
                  value={forms.titulo}
                ...
                <label
                  className="form-label"
                  htmlFor={`subtitle-about-${props.id}`}
                >
                  Subtítulo
                </label>
                <textarea
                  className="form-control rounded-0 h-10"
                  id={`subtitle-about-${props.id}`}
                  name="subtitulo"
                  value={forms.subtitulo}
                  onChange={(evt) => handleChange(evt)}
                />
                <label
                  className="form-label"
                  htmlFor={`description-about-${props.id}`}
                >
                  Descripción
                </label>
                <textarea
                  className="form-control rounded-0 "
                  id={`description-about-${props.id}`}
                  name="descripcion"
                ...
                <label
                  className="form-label"
                  htmlFor={`uploadImage-${props.id}`}
                >
                  Subir imagen
                </label>
                <input
                  className="form-control mb-3"
                  id={`uploadImage-${props.id}`}
                  name="image_url"
                  type="file"
                  onChange={(evt) => handleSetFile(evt)}
                />
                ...
                <div className="btn-group" data-toggle="buttons">
                  <label className="btn btn-secondary">
                    <input
                      ...
                      id="option1"
                      autoComplete="off"
                      onClick={() => setForms({ ...forms, posicion_id: 1 })}
                    />{" "}
                    Izquierda
                  </label>
                  <label className="btn btn-secondary">
                    <input
                      ...
                      id="option2"
                      autoComplete="off"
                      onClick={() => setForms({ ...forms, posicion_id: 2 })}
                    />{" "}
                    Derecha
                  </label>
                </div>
